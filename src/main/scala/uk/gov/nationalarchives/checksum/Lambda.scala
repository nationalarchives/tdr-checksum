...
import graphql.codegen.types.AddFileMetadataWithFileIdInputValues
    ...
    val results = event.getRecords.asScala.toList
      .map(sqsMessage => {
        ...
        val receiptHandleOrError = for {
          ...
          _ <- IO(sendMessage(AddFileMetadataWithFileIdInputValues("SHA256ServerSideChecksum", body.checksumFile.fileId, checksum).asJson.noSpaces))
        } yield body
